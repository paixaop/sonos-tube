...
//var TubeStream = require('./tube-stream');
var ytStream = require('./ytstream');
...
var DESCRIPTION = 'SonosTube - www.github.com/paixaop/sonos-tube';
var URL = 'https://github.com/paixaop/sonos-tube';
...
var SonosTube = function(device, port) {
    ...
    self.dialApps = {
        "YouTube": {
            ...
            launch: function(launchData) {
                ...
                self.newSong('http://www.youtube.com/watch?' + launchData);
            },
            stop: function() {
    ...
    self.fake = function(u) {
        ...
        var screen_id = null;
        r('https://www.youtube.com/gen_204?mse_dw=0&mse_m=true&mse_pp=true&mse_w=0&a=tv_reqs&c=TVHTML5&cver=5.20150715&cbr=Firefox&cbrver=46&cos=Macintosh&cosver=10.11&ctheme=CLASSIC&label=86a41089&appstart=1464836400', function(err, res, body) {
            r('https://www.youtube.com/api/lounge/pairing/generate_screen_id', function(err, res, body) {
              var screen_id = body;
              r.post('https://www.youtube.com/api/lounge/pairing/get_lounge_token_batch',
                       { form: {screen_ids: screen_id}}, function(err, res, body) {
                var screens = JSON.parse(body);
                r.post('https://www.youtube.com/api/lounge/pairing/register_pairing_code',
                    { form: {
                        access_type: 'permanent',
                        app: 'lb-v4',
                        pairing_code: q.pairingCode,
                        screen_id: screen_id,
                        screen_name: SonosTube
                    }},
...
<desc id="cdudn" nameSpace="urn:schemas-rinconnetworks-com:metadata-1-0/">SA_RINCON65031_</desc>
    ...
    self.start = function(callback) {
        ...
        var YTApp = require('./youtube-dial-app');
        var ytApp = new YTApp();
        ytApp.getUUID();
        self.server = http.createServer(self.app).listen(self.listenPort, function(err) {
            if (err) {
                console.log('Error: ' + err.message);
                throw err;
            }
