...
var SonosTube = function(device, port) {
  ...
  self.dialApps = {
    'YouTube': {
      ...
      launch: function(launchData) {
        self.youtubeApp.start(launchData, function(err) {
          if (err) {
            throw err;
          }
          self.inputStream.empty();
          self.newSong(launchData);
        });
  ...
  self.newSong = function(launchData) {
    if (!launchData) {
      throw new Error('newSong must have YouTube\'s launch data as parameter');
    }
    return;
    ...
    try {
      self.queue(ytStream('http://www.youtube.com/watch?v=' + q.v, {
          info: self.updateMetadata,
          format: self.getFormat,
        })
        .pipe(new lame.Decoder()));
