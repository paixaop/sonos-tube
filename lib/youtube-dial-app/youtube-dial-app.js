...
// Fixed YouTube API Parameters
const VERSION = 8;
const APP = 'lb-v4';
const DEVICE = 'LOUNGE_SCREEN';

const BASE_URL = `https://www.youtube.com/api/lounge/bc/bind?device=${DEVICE}&VER=${VERSION}&app=${APP}`;

// URL used in all nomal requests for incoming data
const NORMAL_RECONNECT_URL = `${BASE_URL}&id=%s&name=%s&loungeIdToken=%s&RID=rpc&SID=%s&CI=0&AID=%d&constssionid=%s&TYPE=xmlhttp&zx=%s`;

// URL used in the first request for incoming data
const FIRST_CONNECTION_URL = `${BASE_URL}&id=%s&name=%s&loungeIdToken=%s&RID=%d&zx=%s`;

// URL to get messages after a SID error
const CONNECTION_AFTER_SID_ERROR = `${BASE_URL}&id=%s&name=%s&loungeIdToken=%s&OSID=%s&OAID=%d&RID=%d&zx=%s`;

// URL used to send messages
const SEND_MESSAGE_URL = `${BASE_URL}&id=%s&name=%s&loungeIdToken=%s&SID=%s&RID=%d&AID=%d&gsessionid=%s&zx=%s`;
...
class YouTubeDialApp extends EventEmitter {
  ...
  handleMessage({
    ...
    switch (command) {
      ...
      case 'setPlaylist':
        this.log(`-> setPlaylist: ${args.eventDetails.eventType}: ${args.videoIds}`);
      ...
      case 'stopVideo':
        this.log(`-> stopVideo: {JSON.stringify(args, null, 2)}`);
        break;

      case 'onUserActivity':
        this.log(`-> onUserActivity ${JSON.stringify(args, null, 2)}`);
        break;
  ...
  outgoingBeforeRequest(count, next) {
    ...
    this.log(`[Out] Message: ${JSON.stringify(oprions.postData.form, null, 2)}`);
    next(null, options);
  }
